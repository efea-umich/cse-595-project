...
class TrafficDataset(Dataset):
    def __init__(
        self, path: str, num_sequences=1000, sequence_length=20, pred_horizon=1
    ):
        ...
        while True:
            for series in df["series_value"]:
                for i in range(len(series) - self.sequence_length - self.pred_horizon):
                    self.data.append(
                        torch.tensor(
                            series[i : i + self.sequence_length], dtype=torch.float
                        )
                    )
                    ...
                    self.labels.append(
                        torch.tensor([1.0])
                        if pred_value > series[i + self.sequence_length - 1]
